...
d1 <- matrix(rep(0,(n*N1)),nrow=n)
#WT gene expression matrix
rownames(d1)<-names
colnames(d1)<-paste0("WT",1:N1)

d2 <- matrix(rep(0,(n*N2)),nrow=n)
#WT gene expression matrix
rownames(d2)<-names
...
```{r}
pca <- prcomp(t(cbind(d1,d2)))
plot(pca$x[,c(1,2)],col=c(rep("blue",10),rep("red",10)))
```
...
names(ttest1) <- names
for(i in 1:n){
  foldchange[i] <- mean(d1[i,])-mean(d2[i,])
...
#?p.adjust check the function if you do not know 
	ttest_adj <- p.adjust(ttest1,method="BH")
