...
d1 <- d[rowSums(d[,setdiff(1:dim(d)[2],1)])>20,]
dim(d1)
head(d1)
```
...
```{r}
hist(adj_p_value_lm,breaks=c(0,0.05,0.1,0.4,1))
```
...
var2 <-  rep(10,dim(d1_cpm)[1])

for(i in 1:dim(d1_cpm)[1]){
	var[i]<-shapiro.test(d1_cpm[i,samples_L])$p.value
	var2[i]<-shapiro.test(d1_cpm[i,samples_F])$p.value
}
...
EdgeR dispersions are explained here : https://support.bioconductor.org/p/110273/
